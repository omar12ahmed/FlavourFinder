...
import { useEffect, useState } from "react";
import axios from "axios";
import CocktailRandomApi from "./CocktailRandomApi";
import CocktailModal from "./CocktailModal";
import Typography from "@mui/material/Typography";
import CardContent from '@mui/material/CardContent';
import CardMedia from '@mui/material/CardMedia';
...
function CocktailSearchApi() {
    ...
    const handleLikeToggle = (cocktail) => {
        const index = likedCocktails.findIndex((likedCocktail) => likedCocktail.idDrink === cocktail.idDrink);
        if (index === -1) {
            const updatedLikedCocktails = [...likedCocktails, cocktail];
            setLikedCocktails(updatedLikedCocktails);
            localStorage.setItem("likedCocktails", JSON.stringify(updatedLikedCocktails));
        } else {
            const updatedLikedCocktails = likedCocktails.filter((likedCocktail) => likedCocktail.idDrink !== cocktail.idDrink);
            setLikedCocktails(updatedLikedCocktails);
            localStorage.setItem("likedCocktails", JSON.stringify(updatedLikedCocktails));
        }
    };
